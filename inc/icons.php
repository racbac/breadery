<?php 
/* Adapted from WordPress's Twenty Nineteen theme at https://wordpress.org/themes/twentynineteen
 * See twentynineteen/inc/icon-functions.php and twentynineteen/inc/class-twentynineteen-svg-icons.php
 * Supports Font Awesome webfont instead of SVGs
 */
class Breadery_FA_Icons {

	/* Gets the code for a given icon */
	public static function get_icon_fa($group, $icon) {
		if ( 'solid' == $group ) {
			$arr = self::$solid_icons;
			$cat = "fas";
		} elseif ( 'brand' == $group ) {
			$arr = self::$brand_icons;
			$cat = 'fab';
		} else {
			return null;
		}
		if ( in_array( $icon, $arr ) ) {
			$fa  = trim("<i class='".$cat." fa-".$icon."'></i>");
			$fa  = preg_replace( "/([\n\t]+)/", ' ', $fa ); // Remove newlines & tabs.
			$fa  = preg_replace( '/>\s*</', '><', $fa ); // Remove white space between tags.
			return $fa;
		}
		return null;
	}

	/* Detect social network from url, return corresponding Font Awesome icon */
	public static function get_social_link_fa($uri) {
		static $regex_map; // Only compute regex map once, for performance.
		if ( ! isset( $regex_map ) ) {
			$regex_map = array();
			$domain_map       = &self::$domain_icon_map; // Use reference instead of copy, to save memory.
			$icon_map = &self::$network_icon_map;
			foreach ( self::$social_networks as $network ) {
				$domains = array_key_exists( $network, $domain_map ) ? $domain_map[ $network ] : array( sprintf( '%s.com', $network ) );
				...
				$icon = array_key_exists($network, $icon_map) ? $icon_map[$network][0] . ' ' . $icon_map[$network][1] : 'brand '.$network;
				$regex_map[ $icon ] = sprintf( '/(%s)/i', implode( '|', $domains ) );
			}
		}
		foreach ( $regex_map as $icon => $regex ) {
			if ( preg_match( $regex, $uri ) ) {
				return self::get_icon_fa(substr($icon, 0, 5) , substr($icon,6));
			}
		}
		return null;
	}
